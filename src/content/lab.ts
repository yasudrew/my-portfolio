...
const RAW: readonly LabEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "ほかの演出と違って、これは状態ではなく出来事です。1秒で終わるので、カーソルを置いているあいだは繰り返し再生しています。",
    ],
  ...
  },
  {
    ...
    body: [
      ...
      "Hero Objectを照らしているのがこれです。あの球の表面に映り込んでいるものには、外部のファイルが一枚も使われていません。",
    ],
    source: "src/gl/env/studioEnvironment.ts",
    takeaways: ["Code"],
  },
  {
    slug: "hero-object",
    ...
    kind: "Shader",
    status: "live",
    lede: "ノイズで歪みつづける球。ポインタのほうへ少し傾きます。",
    body: [
      "最初の扉絵です。球の表面を simplex ノイズで押し出して、ゆっくり回しながらポインタの方向へわずかに傾けています。端末の性能に応じて分割数を落とす作りです。",
      "頂点を動かすと、もとの法線は嘘になります。ここでは接平面上の二点をずらして、その三点から法線を毎フレーム作り直しています。これをやらないと、ハイライトが表面に乗らず、上を滑っていくように見えます。",
      "この一覧で唯一、全画面の板ではなく本物の立体です。キャンバスが深度バッファを持っているのは、この球の裏側が表側の上に描かれないようにするためだけです。",
      "見た目は悪くなかったのですが、扉絵としては「何のサイトか」が一切伝わらないので落としました。",
    ],
    source: "src/gl/scenes/lab/HeroObject.tsx",
    takeaways: ["Code", "Demo"],
  },
];
